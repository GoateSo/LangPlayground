...
class UtilSuite extends FunSuite:
  // tests for convert
  test("convert unop") {
    val results = Utils.convert(UnOp("-", Num(1))).replace(" ", "")
    assertEquals(results, "(-1.0)")
  }
  test("convert unop with parens") {
    val results = Utils.convert(UnOp("-", UnOp("-", Num(1)))).replace(" ", "")
    assertEquals(results, "(-(-1.0))")
  }
  test("convert binop") {
    val results = Utils.convert(BinOp("+", Num(1), Num(2))).replace(" ", "")
  ...
  test("printAST unop") {
    ...
    Console.withOut(out) {
      Utils.dispAST(UnOp("-", Num(1)))
    }
    val output = out.toString().replace("\t"," ").split("\n").map(_.strip()).toList
    assertEquals(output, """
    UnOp(-)
    | Num(1.0)
    """.strip().split("\n").map(_.strip()).toList)
  ...
  test("printAST binop") {
    val out = new java.io.ByteArrayOutputStream()
    Console.withOut(out) {
      Utils.dispAST(BinOp("+", Num(1), Num(2)))
    }
    val output = out.toString().replace("\t"," ").split("\n").map(_.strip()).toList
    assertEquals(output, """
    BinOp(+)
  ...
  test("printAST variable definition with function call"){
    val out = new java.io.ByteArrayOutputStream()
    Console.withOut(out) {
      Utils.dispAST(VarDef("x", FunCall("f", List(Num(1)))))
    }
    val output = out.toString().replace("\t"," ").split("\n").map(_.strip()).toList
    assertEquals(output, """
    VarDef(x)
  ...
  test("printAST function definition"){
    val out = new java.io.ByteArrayOutputStream()
    Console.withOut(out) {
      Utils.dispAST(FunDef("f", List("x"), Num(1)))
    }
    val output = out.toString().replace("\t"," ").split("\n").map(_.strip()).toList
    assertEquals(output, """
    FunDef(f)
  ...
  test("printAST function call"){
    val out = new java.io.ByteArrayOutputStream()
    Console.withOut(out) {
      Utils.dispAST(FunCall("f", List(Num(1))))
    }
    val output = out.toString().replace("\t"," ").split("\n").map(_.strip()).toList
    assertEquals(output, """
    FunCall(f)
